...
export function authRoutes(db: Db): Router {
  ...
  const enforceLimit = (req: Request, accountKey: string) => {
    ...
    const wait = Math.max(
      perIp.retryAfter("ip:" + ip),
      perAccount.retryAfter(accountKey),
    );
    if (wait > 0) {
      throw new HttpError(
        429,
  ...
  router.get("/status", (_req, res) => {
    res.json({
      ...
      // So the forms state the real rule rather than a hardcoded number that
      // drifts the moment the policy changes.
  ...
  router.get(
    ...
    asyncRoute(async (req, res) => {
      ...
      const pending = readSignedPayload<PendingAuth>(
        req.cookies?.[OAUTH_COOKIE],
      );
      ...
      if (state !== pending.state)
        return fail("Sign-in request did not match. Please try again.");

      try {
        const { idToken } = await exchangeCode(config, code, pending.verifier);
        const profile = validateClaims(
          await readIdToken(idToken),
          config,
          pending.nonce,
        );
        ...
        return fail(
          err instanceof HttpError ? err.message : "Google sign-in failed.",
        );
      }
    }),
  );
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      await assertAcceptablePasswordAsync(input.newPassword);
      ...
      auditAnonymous(
        db,
        req,
        { action: "auth.reset_redeemed" },
        { userId, email: emailOf(db, userId) },
      );
  ...
  router.post("/login", (req, res) => {
    ...
    if (!actor)
      throw new HttpError(500, "Could not load the signed-in account.");
    res.json(buildSnapshot(db, actor));
  });
  ...
  router.post(
    ...
    asyncRoute(async (req, res) => {
      if (req.actor!.previewAsId) {
        throw new HttpError(
          400,
          "Exit the preview before changing your password.",
        );
      }
  ...
  router.post("/preview", requireAuth, requireTeamAdmin, (req, res) => {
    ...
    if (teammateId === req.actor!.userId)
      throw new HttpError(400, "You are already yourself.");
