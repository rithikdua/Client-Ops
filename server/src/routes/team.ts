...
function grantedList(db: Db, userId: string): string {
  ...
  const granted = ACCESS_SECTIONS.filter((s) => access[s.key]).map((s) => s.key);
...
export function teamRoutes(db: Db): Router {
  ...
  router.put('/:userId/access', requireSection('team'), requireTeamAdmin, (req, res) => {
    ...
    const user = db.prepare('SELECT id, email, permission FROM users WHERE id = ?').get(userId) as
  ...
  router.delete('/:userId', requireSection('team'), requireTeamAdmin, (req, res) => {
    ...
    const user = db.prepare('SELECT name, email, permission FROM users WHERE id = ?').get(userId) as
